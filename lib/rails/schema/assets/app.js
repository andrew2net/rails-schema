(function() {
  ...
  // Sidebar
  function buildModelItem(n, filtered) {
  ...
  function buildSidebar() {
    ...
    if (config.grouping_enabled) {
      ...
      function buildGroupHeader(models, labelHtml) {
        var header = document.createElement("div");
        header.className = "group-header";
        header.innerHTML = '<span class="group-toggle">&#9660;</span>' + labelHtml;
        ...
        header.addEventListener("dblclick", function(e) {
          ...
          var allGroupVisible = models.every(function(n) { return visibleModels.has(n.id); });
          if (visibleModels.size === nodes.length) {
            visibleModels.clear();
            models.forEach(function(n) { visibleModels.add(n.id); });
          } else if (allGroupVisible) {
      ...
      sortedGroups.forEach(function(key) {
        ...
        var header = buildGroupHeader(group.models, labelHtml);
        list.appendChild(header);

        var groupContainer = document.createElement("div");
        groupContainer.className = "group-models";
        group.models.forEach(function(n) {
          groupContainer.appendChild(buildModelItem(n, filtered));
        });
      ...
      if (ungrouped.length > 0 && sortedGroups.length > 0) {
        ...
        var ungroupedHeader = buildGroupHeader(ungrouped, ungroupedLabelHtml);
