(function() {
  ...
  function ticked() {
    edgeGroup.selectAll(".edge-group").each(function(d) {
      ...
      var y1 = pts.y1 + parallelOffset;
      var y2 = pts.y2 + parallelOffset;

      // Compute cubic bezier control points
      var cp1x, cp1y, cp2x, cp2y;
      if (pts.sameDirection) {
        var loopOffset = Math.max(60, Math.abs(y2 - y1) * 0.5);
        cp1x = pts.x1 + loopOffset;
        cp1y = y1;
        cp2x = pts.x2 + loopOffset;
        cp2y = y2;
      } else {
        var dx = pts.x2 - pts.x1;
        var cpOffset = Math.max(50, Math.abs(dx) * 0.4);
        var dir = Math.sign(dx) || 1;
        cp1x = pts.x1 + cpOffset * dir;
        cp1y = y1;
        cp2x = pts.x2 - cpOffset * dir;
        cp2y = y2;
      }
      ...
      var labelX = 0.125 * pts.x1 + 0.375 * cp1x + 0.375 * cp2x + 0.125 * pts.x2;
      var labelY = (y1 + y2) / 2;
      g.select("text")
        .attr("x", labelX)
        .attr("y", labelY);
